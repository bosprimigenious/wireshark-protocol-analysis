...
#keybox[过滤器与命令][
  - Capture Filter：`tcp port 443`（或 `tcp port 80`）
  - Display Filter：`tcp.port == 443`
  - 触发流量：浏览器访问 `https://www.baidu.com`
]

保存为 `captures/tcp/tcp-https.pcapng`，确认含 SYN 握手、数据段与 FIN 挥手。
...
#figure(
  table(
    ...
    [Source Port], [16 bit], [62451], [源端口],
    [Destination Port], [16 bit], [443], [HTTPS 端口],
...
#figure(
  table(
    ...
    [2], [Server→Client], [SYN, ACK], [208860029], [3410032807], [Ack = Seq+1 ✓],
    [3], [Client→Server], [ACK], [3410032807], [208860030], [Ack = Seq+1 ✓],
  ),
  caption: [TCP 三次握手],
)

#assetshot("tcp-handshake.png", [TCP 三次握手字段摘录])
...
#figure(
  ...
  sequenceDiagram
    ...
    C->>S: SYN Seq=3410032806
    S->>C: SYN+ACK Seq=208860029 Ack=3410032807
    C->>S: ACK Seq=3410032807 Ack=208860030
  ```,
  caption: [TCP 三次握手时序图],
)
...
帧 32 客户端发送 `Seq=3410032807, Len=460`；帧 33 服务器 `Ack=3410033267`。

验证：`3410033267 = 3410032807 + 460` ✓。帧 40 与帧 38 同 Seq，为*超时重传*。

+ *Window Size*（帧 33）：948 字节，接收方流量控制窗口。
+ *MSS*：客户端 1460，服务器 1382（SYN 选项协商）。

#assetshot("tcp-data.png", [TCP 数据传输与确认摘录])
...
#figure(
  ...
  sequenceDiagram
    participant C as Client
    participant S as Server
    C->>S: PSH+ACK Seq=3410032807 Len=460
    S->>C: ACK Ack=3410033267 Win=948
    S->>C: RETRANS Seq=208864110
  ```,
  caption: [TCP 数据传输与重传示意],
)

== 四次挥手

帧 55–59，连接以帧 58 RST 结束（HTTPS 短连接常见）：
...
#figure(
  table(
    ...
    [1], [55], [Client→Server], [FIN, ACK], [3410033530], [208868323], [请求关闭],
    [2], [56], [Server→Client], [ACK], [208868323], [3410033530], [服务器确认],
    [3], [59], [Server→Client], [FIN, ACK], [208868354], [3410033530], [服务器关闭],
    [4], [58], [Client→Server], [RST, ACK], [3410033531], [208868354], [RST 复位],
  ),
  caption: [TCP 连接释放（含 RST）],
)

#assetshot("tcp-teardown.png", [TCP 四次挥手字段摘录])
...
#figure(
  ...
  sequenceDiagram
    participant C as Client
    participant S as Server
    C->>S: FIN+ACK Seq=3410033530
    S->>C: ACK Ack=3410033530
    S->>C: FIN+ACK Seq=208868354
    C->>S: RST+ACK Seq=3410033531
  ```,
  caption: [TCP 连接释放时序图（含 RST）],
)
...
+ *TCP 使用 HTTPS 443*\
  现象：80 端口无流量，改用 `curl https://www.baidu.com`。\
  解决：载荷虽加密，SYN/ACK/FIN 及 Seq/Ack/Window 均可完整分析。
